...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            add2() {
                let self = this;
                if(Tool.isEmpty(self.active)){
                    Toast.warning("请先选择一级分类");
                    return;
                }
                self.category = {
                    parent: self.active.id
                };
                $("#form-modal").modal("show");
            },
            ...
            all() {
                ...
                self.$ajax.post(process.env.VUE_APP_SERVER + '/business/admin/category/all').then((response) => {
                    ...
                    // 将所以记录格式化为树形结构
                    self.level1 = [];
                    for (let i = 0; i < self.categorys.length; i++) {
                        let c = self.categorys[i];
                        if (c.parent === '00000000') {
                            self.level1.push(c)
                            for (let j = 0; j < self.categorys.length; j++) {
                                let child = self.categorys[j];
            ...
            save() {
                ...
                // 保存校验
                if (1 != 1
                    || !Validator.require(self.category.parent, "父id")
                    || !Validator.require(self.category.name, "名称")
                    || !Validator.length(self.category.name, "名称", 1, 50)
                ) {
                    return;
                }

                Loading.show();
                self.$ajax.post(process.env.VUE_APP_SERVER + '/business/admin/category/save', self.category).then((response) => {
                    Loading.hide();
                    let resp = response.data;
                    if (resp.success) {
            ...
            del(id) {
                ...
                Confirm.show("删除分类后不可恢复，确认删除？", function () {
                    Loading.show();
                    self.$ajax.delete(process.env.VUE_APP_SERVER + '/business/admin/category/delete/' + id).then((response) => {
                        Loading.hide();
                        let resp = response.data;
                        if (resp.success) {
                            self.all();
