    ...
    export default {
        ...
        methods: {
            ...
            list(page) {
                let self = this;
                Loading.show();
                self.$ajax.post(process.env.VUE_APP_SERVER + '/business/admin/teacher/list', {
                    page: page,
                    size: self.$refs.pagination.size,
                }).then((response) => {
                    Loading.hide();
                    let resp = response.data;
                    self.teachers = resp.content.list;
            ...
            save() {
                ...
                // 保存校验
                if (1 != 1
                    || !Validator.require(self.teacher.name, "姓名")
                    || !Validator.length(self.teacher.name, "姓名", 1, 50)
                    || !Validator.length(self.teacher.nickname, "昵称", 1, 50)
                    || !Validator.length(self.teacher.image, "头像", 1, 100)
                    || !Validator.length(self.teacher.position, "职位", 1, 50)
                    || !Validator.length(self.teacher.motto, "座右铭", 1, 50)
                    || !Validator.length(self.teacher.intro, "简介", 1, 500)
                ) {
                    return;
                }

                Loading.show();
                self.$ajax.post(process.env.VUE_APP_SERVER + '/business/admin/teacher/save', self.teacher).then((response) => {
                    Loading.hide();
                    let resp = response.data;
                    if (resp.success) {
            ...
            del(id) {
                ...
                Confirm.show("删除讲师后不可恢复，确认删除？", function () {
                    Loading.show();
                    self.$ajax.delete(process.env.VUE_APP_SERVER + '/business/admin/teacher/delete/' + id).then((response) => {
                        Loading.hide();
                        let resp = response.data;
                        if (resp.success) {
                            self.list(1);
