    ...
    <div>
        ...
        <table id="simple-table" class="table  table-bordered table-hover">
            ...
            <tr v-for="file in files">
                ...
                <td>{{file.name}}</td>
                <td>{{file.suffix}}</td>
                <td>{{file.size}}</td>                <td>{{FILE_USE | optionKV(file.use)}}</td>
    ...
    export default {
        ...
        methods: {
            ...
            list(page) {
                let self = this;
                Loading.show();
                self.$ajax.post(process.env.VUE_APP_SERVER + '/file/admin/file/list', {
                    page: page,
                    size: self.$refs.pagination.size,
                }).then((response) => {
                    Loading.hide();
                    let resp = response.data;
                    self.files = resp.content.list;
            ...
            save() {
                ...
                // 保存校验
                if (1 != 1
                    || !Validator.require(self.file.path, "相对路径")
                    || !Validator.length(self.file.path, "相对路径", 1, 100)
                    || !Validator.length(self.file.name, "文件名", 1, 100)
                    || !Validator.length(self.file.suffix, "后缀", 1, 10)
                    || !Validator.length(self.file.key, "文件标识", 1, 32)
                ) {
                    return;
                }

                Loading.show();
                self.$ajax.post(process.env.VUE_APP_SERVER + '/file/admin/file/save', self.file).then((response) => {
                    Loading.hide();
                    let resp = response.data;
                    if (resp.success) {
            ...
            del(id) {
                ...
                Confirm.show("删除文件后不可恢复，确认删除？", function () {
                    Loading.show();
                    self.$ajax.delete(process.env.VUE_APP_SERVER + '/file/admin/file/delete/' + id).then((response) => {
                        Loading.hide();
                        let resp = response.data;
                        if (resp.success) {
                            self.list(1);
