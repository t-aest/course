    ...
    export default {
        ...
        methods: {
            ...
            list(page) {
                let self = this;
                Loading.show();
                self.$ajax.post(process.env.VUE_APP_SERVER + '/business/admin/section/list', {
                    page: page,
                    ...
                    self.sections = resp.content.list;
            ...
            save() {
                ...
                self.section.courseId = self.course.id
                self.section.chapterId = self.chapter.id
                Loading.show();
            ...
            del(id) {
                ...
                Confirm.show("删除小节后不可恢复，确认删除？", function () {
                    Loading.show();
                    self.$ajax.delete(process.env.VUE_APP_SERVER + '/business/admin/section/delete/' + id).then((response) => {
                        Loading.hide();
                        let resp = response.data;
                        if (resp.success) {
                            self.list(1);
